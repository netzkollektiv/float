...
class WC_PDF_Product_Vouchers_Redemption_Handler {
		...
		if ( $valid && $this->is_voucher_coupon( $coupon ) && $voucher = wc_pdf_product_vouchers_get_voucher( $coupon->get_code() ) ) {
			...
			if ( 'multi' === $voucher->get_voucher_type() ) {
				...
				if ( $voucher->get_voucher_currency() !== $currency ) {
					throw new Framework\SV_WC_Plugin_Exception( sprintf( __( 'Sorry, this voucher is only valid for the %s currency.', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_currency() ), 100 );
				}
			}
		}
	...
	private function should_hide_tax_label() {
		 ...
		 * @param \WC_PDF_Product_Vouchers_Redemption_Handler $handler the PDF Product Vouchers redemption handler instance
		 */
