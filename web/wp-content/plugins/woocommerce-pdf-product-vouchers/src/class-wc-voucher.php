...
class WC_Voucher extends \WC_Voucher_Base {
	...
	public function redeem( $redemption = null, $args = array() ) {
		...
		if ( $remaining_val < $amt_to_redeem ) {
			throw new Framework\SV_WC_Plugin_Exception( __( 'Redemption amount is greater than remaining voucher value', 'woocommerce-pdf-product-vouchers' ) );
		}
		...
		if ( 'single' === $this->get_voucher_type() && ( $amt_to_redeem < $product_price || ( round( $amt_to_redeem / $product_price ) > $remaining_qty ) ) ) {
			throw new Framework\SV_WC_Plugin_Exception( __( 'Redemption amount must be a multiple of the product price for a single-purpose voucher.', 'woocommerce-pdf-product-vouchers' ) );
		}
		...
		foreach ( $redemptions as $i => $redemption ) {
			...
			if ( $is_single ) {
				...
				if ( $remainder > 0 && (string) $remainder !== (string) $product_price ) {
					throw new Framework\SV_WC_Plugin_Exception( sprintf( '%s %s', $invalid_amount_message, __( 'Redemption amount must be a multiple of the product price for a single-purpose voucher.', 'woocommerce-pdf-product-vouchers' ) ) );
				}
			}
			...
			if ( $redeemed > $value ) {
				throw new Framework\SV_WC_Plugin_Exception( __( 'Total amount of redemptions cannot be greater than voucher value.', 'woocommerce-pdf-product-vouchers' ) );
			}
