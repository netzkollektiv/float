 ...
 * @since 3.5.0
 */
class WC_PDF_Product_Vouchers_Redeem_Multi implements WC_PDF_Product_Vouchers_Redeem {
	...
	public function redeem( $voucher, $amount ) {
		...
		if ( $amount > 0 ) {
			...
			$message_text = esc_html__( 'Voucher %s was redeemed for %s!%sThis voucher has %s remaining', 'woocommerce-pdf-product-vouchers' );
			...
			$message_text = esc_html__( 'Voucher %s requires an amount for redemption!%sThis voucher has %s available. Enter an amount to redeem, or %sclick here%s to view the voucher.', 'woocommerce-pdf-product-vouchers' );
		...
		$response = new WC_PDF_Product_Vouchers_Redeem_Response_Multi();
