...
class WC_PDF_Product_Vouchers_AJAX {
	...
	public function add_voucher_note() {
		...
		try {
			...
			if ( ! $voucher ) {
				throw new Framework\SV_WC_Plugin_Exception( __( 'Undefined voucher to add note to.', 'woocommerce-pdf-product-vouchers' ) );
			}

			$note_text = wp_kses_post( trim( stripslashes( $_POST['note'] ) ) );

			if ( '' === $note_text ) {
				throw new Framework\SV_WC_Plugin_Exception( __( 'A voucher note should not be empty.', 'woocommerce-pdf-product-vouchers' ) );
			}

			$comment_id = $voucher->add_note( $note_text );

			if ( ! $comment_id ) {
				throw new Framework\SV_WC_Plugin_Exception( __( 'Please ensure the voucher has been created and saved before adding a note to it.', 'woocommerce-pdf-product-vouchers' ) );
			}
	...
	public function barcode_redeem_voucher() {
		...
		try {
			...
			switch ( $voucher->get_voucher_type() ) {
				...
				case 'single':
					$redemption_handler = new WC_PDF_Product_Vouchers_Redeem_Single();
					$amount             = $voucher->get_product_price();
				break;

				case 'multi':
					$redemption_handler = new WC_PDF_Product_Vouchers_Redeem_Multi();
					$amount             = isset( $_POST['amount'] ) ? (float) $_POST['amount'] : null;
				break;

				default:
					/* translators: Unknown voucher type. Placeholders: %1$s - voucher type */
					$message = sprintf( esc_html__( "Voucher type '%s' wasn't recognized.", 'woocommerce-pdf-product-vouchers' ) , $voucher->get_voucher_type() );
	...
	private function is_redeemable( $voucher ) {
		...
		if ( ! $voucher ) {
			...
			throw new Framework\SV_WC_Plugin_Exception(
				...
				esc_html__( 'Voucher was not found!', 'woocommerce-pdf-product-vouchers' )
			);
		...
		if ( ! $voucher->is_editable() ) {
			...
			$message = __( 'Voucher %s is not redeemable! %sClick Here%s to view the voucher!', 'woocommerce-pdf-product-vouchers' );
			...
			if ( 'redeemed' === $voucher->get_status() ) {
				...
				$message = __( 'Voucher %s was already redeemed! %sClick Here%s to view the voucher!', 'woocommerce-pdf-product-vouchers' );
