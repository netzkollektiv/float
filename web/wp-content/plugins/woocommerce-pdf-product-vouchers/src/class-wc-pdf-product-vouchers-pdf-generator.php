...
class WC_PDF_Product_Vouchers_PDF_Generator {
	...
	public static function generate_voucher_pdf( \WC_Voucher $voucher, $save = true ) {
		...
		if ( ! $image ) {
			throw new Framework\SV_WC_Plugin_Exception( __( 'Voucher image not found', 'woocommerce-pdf-product-vouchers' ) );
		}

		// make sure the file exists and is readable
		if ( ! is_readable( $voucher->get_image_path() ) ) {
			/* translators: Placeholders: %s - image path */
			throw new Framework\SV_WC_Plugin_Exception( sprintf( __( 'Voucher image file missing or not readable: %s', 'woocommerce-pdf-product-vouchers' ), $upload_dir['basedir'] . '/' . $image['file'] ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( sprintf( __( "Cannot load voucher HTML: %s", 'woocommerce-pdf-product-vouchers' ), $response->get_error_message() ) );
		}
		...
		if ( isset( $response['response']['code'] ) && 200 !== (int) $response['response']['code'] ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( sprintf( __( 'Cannot load voucher HTML: %1$d - %2$s', 'woocommerce-pdf-product-vouchers' ), $response['response']['code'], $response['response']['message'] ) );
		}

		$html = wp_remote_retrieve_body( $response );

		if ( empty( $html ) ) {
			throw new Framework\SV_WC_Plugin_Exception( __( 'Voucher HTML is empty', 'woocommerce-pdf-product-vouchers' ) );
		}
		...
		try {
			...
			throw new Framework\SV_WC_Plugin_Exception( sprintf( __( 'Cannot create voucher: %s', 'woocommerce-pdf-product-vouchers' ), $e->getMessage() ) );
		}
		...
		if ( ! is_writable( $voucher_path ) ) {
			...
			throw new Framework\SV_WC_Plugin_Exception( sprintf( __( 'Voucher path %s is not writable', 'woocommerce-pdf-product-vouchers' ), $voucher_path ) );
		}
