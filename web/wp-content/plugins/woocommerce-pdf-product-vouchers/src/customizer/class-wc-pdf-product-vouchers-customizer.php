...
class WC_PDF_Product_Vouchers_Customizer {
	...
	public function add_customizer_controls( $wp_customize ) {
		...
		$wp_customize->register_control_type( 'WC_PDF_Product_Vouchers_Barcode_Customize_Position_Control' );
			...
			foreach ( $controls as $control_key => $attrs ) {
				...
				switch ( $type ) {
					...
					case 'wc_pdf_product_vouchers_barcode_position':
						$wp_customize->add_control( new \WC_PDF_Product_Vouchers_Barcode_Customize_Position_Control( $wp_customize, $control_id, $attrs ) );
					break;
	...
	public function customizer_controls_scripts() {
		...
		wp_localize_script( 'woocommerce-pdf-product-vouchers-customizer-control-scripts', 'woocommerce_vouchers_admin_customizer', array(
			...
			'i18n'                   => [
				...
				'something'              => sprintf( __( '%s Only QR code scanning is supported from the voucher admin at this time.' ), '<span class="dashicons dashicons-warning"></span>' ),
			],
		) );
