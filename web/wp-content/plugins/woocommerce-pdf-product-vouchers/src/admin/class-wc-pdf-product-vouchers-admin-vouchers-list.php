	...
	public function add_voucher_filters( $post_type, $which ) {
		...
		if ( 'wc_voucher' === $post_type ) {
			...
			<select
				...
				data-placeholder='<?php esc_attr_e( 'Filter by voucher template', 'woocommerce-pdf-product-vouchers' ); ?>'
	...
	public function custom_voucher_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'remaining_value':
				...
				$display_remaining = wc_price( $voucher->get_remaining_value_for_display(), [ 'currency' => $voucher->get_voucher_currency() ] );
