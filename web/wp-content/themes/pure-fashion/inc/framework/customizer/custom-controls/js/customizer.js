jQuery(function($) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  thb_GetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function thb_GetAllImageCheckboxes($element) {
	...
	$('.google-fonts-list').on('change', function() {
		...
		var indexes = $.map(bodyfontcontrol.thb_fontslist, function(obj, index) {
		...
		$.each(bodyfontcontrol.thb_fontslist[index].variants, function(val, text) {
		...
		$(this).parent().parent().find('.google-fonts-category').val(bodyfontcontrol.thb_fontslist[index].category);

		thb_GetAllSelects($(this).parent().parent());
	});
	...
	function thb_GetAllSelects($element) {
		var selectedFont = {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].thb_tinymcetoolbar1;
	...
	$( '.alpha-color-control' ).each( function() {
		...
		colorPickerOptions = {
			change: function( event, ui ) {
				...
				if ( defaultColor == value ) {
					alpha = thb_get_alpha_value_from_color( value );
		...
		alphaVal = thb_get_alpha_value_from_color( startingColor );
		...
		$container.find( '.min-click-zone' ).on( 'click', function() {
			thb_update_alpha_value_on_color_control( 0, $control, $alphaSlider, true );
		});
		$container.find( '.max-click-zone' ).on( 'click', function() {
			thb_update_alpha_value_on_color_control( 100, $control, $alphaSlider, true );
		});
		...
		$container.find( '.iris-palette' ).on( 'click', function() {
			...
			alpha = thb_get_alpha_value_from_color( color );

			thb_update_alpha_value_on_alpha_slider( alpha, $alphaSlider );
		...
		$container.find( '.button.wp-picker-clear' ).on( 'click', function() {
			...
			thb_update_alpha_value_on_alpha_slider( 100, $alphaSlider );
		});

		// Bind event handler for clicking on the 'Default' button.
		$container.find( '.button.wp-picker-default' ).on( 'click', function() {
			var alpha = thb_get_alpha_value_from_color( defaultColor );
		...
		$control.on( 'input', function() {
			var value = $( this ).val();
			var alpha = thb_get_alpha_value_from_color( value );
		...
		$alphaSlider.slider().on( 'slide', function( event, ui ) {
			...
			thb_update_alpha_value_on_color_control( alpha, $control, $alphaSlider, false );
	 ...
	 */
	function thb_get_alpha_value_from_color( value ) {
		var alphaVal;
	 ...
	 */
	 function thb_update_alpha_value_on_color_control( alpha, $control, $alphaSlider, update_slider ) {
	 ...
	 */
	function thb_update_alpha_value_on_alpha_slider( alpha, $alphaSlider ) {
