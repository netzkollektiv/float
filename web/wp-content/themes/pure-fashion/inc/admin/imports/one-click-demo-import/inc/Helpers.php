...
class Helpers {
	...
	public static function download_import_files( $import_file_info, $thb_import_options ) {
		...
		if ( array_key_exists( 'ty-theme-options', $thb_import_options ) ) {
			...
			if ( ! empty( $import_file_info['import_theme_options_url'] ) ) {
				// Setup filename path to save the theme options content.
				...
				// Download the theme options import file.
		...
		if ( array_key_exists( 'ty-customizer', $thb_import_options ) ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'An error occurred while writing file to your server! Tried to write a file to: %1$s%2$s.', 'pure-fashion' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					// translators: %1$s file path
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %1$s%2$s.', 'pure-fashion' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'This WordPress page does not have %1$sdirect%2$s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %3$s.', 'pure-fashion' ),
					'<strong>',
					'</strong>',
		...
		$plugin_page_setup = apply_filters(
			...
			array(
				...
				'page_title'  => esc_html__( 'One Click Demo Import', 'pure-fashion' ),
				'menu_title'  => esc_html__( 'Import Demo Data', 'pure-fashion' ),
				'capability'  => 'import',
		...
		$creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null );
		if ( false === $creds ) {
			return new \WP_error(
				'filesystem_credentials_could_not_be_retrieved',
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'pure-fashion' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'pure-fashion' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'pt-ocdi/attachment_prefix', esc_html__( 'One Click Demo Import - ', 'pure-fashion' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					// translators: %1$s html tags.
					esc_html__( '%1$sYour user role isn\'t high enough. You don\'t have permission to import demo data.%2$s', 'pure-fashion' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		$content_file_info    = isset( $_FILES['content_file'] ) ? wp_handle_upload( $_FILES['content_file'], $upload_overrides ) : false;
		$widget_file_info     = isset( $_FILES['widget_file'] ) ? wp_handle_upload( $_FILES['widget_file'], $upload_overrides ) : false;
		$customizer_file_info = isset( $_FILES['customizer_file'] ) ? wp_handle_upload( $_FILES['customizer_file'], $upload_overrides ) : false;
		...
		if ( $content_file_info && ! isset( $content_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					__( 'Content file was not uploaded. Error: %s', 'pure-fashion' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'pure-fashion' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					__( 'Widget file was not uploaded. Error: %s', 'pure-fashion' ),
					$widget_file_info['error']
				),
				$log_file_path,
				esc_html__( 'Upload files', 'pure-fashion' )
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					// translators: %s error.
					esc_html__( 'Customizer file was not uploaded. Error: %s', 'pure-fashion' ),
		...
		$log_added = self::append_to_file(
			esc_html__( 'The import files were successfully uploaded!', 'pure-fashion' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files', 'pure-fashion' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		...
		return PHP_EOL .
			sprintf(
				...
				esc_html__( 'Initial max execution time = %s', 'pure-fashion' ),
				ini_get( 'max_execution_time' )
			) . PHP_EOL .
			sprintf(
				// translators: %s file names.
				__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s', 'pure-fashion' ),
				PHP_EOL,
				get_site_url(),
				empty( $selected_import_files['content'] ) ? esc_html__( 'not defined!', 'pure-fashion' ) : $selected_import_files['content'], // phpcs:ignore
				empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'pure-fashion' ) : $selected_import_files['widgets'], // phpcs:ignore
				empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'pure-fashion' ) : $selected_import_files['customizer'], // phpcs:ignore
				empty( $redux_file_string ) ? esc_html__( 'not defined!', 'pure-fashion' ) : $redux_file_string // phpcs:ignore
			);
	}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = gmdate( apply_filters( 'pt-ocdi/date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
